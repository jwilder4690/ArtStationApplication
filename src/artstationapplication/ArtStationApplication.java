...
public class ArtStationApplication extends PApplet {
	...
	protected PSurface initSurface() {
		...
		/*
		 * Dev note: I used both lambda expressions and EventHandler class creation for
		 * practice. Typically all expression handling should be consistent.
		 */
		...
		gui.saveAs.setOnAction(event -> {
			...
			File initialDirectory = new File(
					"C:\\Users\\wilder4690\\Documents\\GitHub\\ArtStationApplication\\src\\artstationapplication\\data\\saveFiles");
			fileChooser.setInitialDirectory(initialDirectory);
			fileChooser.setTitle("Save Drawing");
			fileChooser.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("ASF", "*.txt"));
			File file = fileChooser.showSaveDialog(stage); // arg here is a Window that input will be blocked to until
															// dialog complete (can be null)
			if (file != null) {
		...
		gui.open.setOnAction(event -> {
			...
			fileChooser.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("ASF", "*.txt"));
			File file = fileChooser.showOpenDialog(stage); // arg here is a Window that input will be blocked to until
															// dialog complete (can be null)
			if (file != null) {
				String location = file.getAbsolutePath();
		...
		EventHandler<ActionEvent> MenuHandler = new EventHandler<ActionEvent>() {
			public void handle(ActionEvent ae) {
				...
				if (name.equals("Exit"))
					Platform.exit();
				else if (target == gui.clipboardFile)
					exportProcessingFileToClipboard();
				else if (target == gui.clipboardShapes)
					exportProcessingShapesToClipboard();
					...
					File file = fileChooser.showSaveDialog(stage); // arg here is a Window that input will be blocked to
																	// until dialog complete (can be null)
					if (file != null) {
						String location = file.getAbsolutePath();
		...
		EventHandler<ActionEvent> gridToggle = new EventHandler<ActionEvent>() {
			public void handle(ActionEvent ae) {
				if (ae.getSource() == gui.cbGridOn)
					gui.gridOn.setSelected(gui.cbGridOn.isSelected());
				else
					gui.cbGridOn.setSelected(gui.gridOn.isSelected());
		...
		EventHandler<ActionEvent> snapToggle = new EventHandler<ActionEvent>() {
			public void handle(ActionEvent ae) {
				if (ae.getSource() == gui.cbGridSnap)
					gui.gridSnapOn.setSelected(gui.cbGridSnap.isSelected());
				else
					gui.cbGridSnap.setSelected(gui.gridSnapOn.isSelected());
		...
		EventHandler<ActionEvent> textHandler = new EventHandler<ActionEvent>() {
			public void handle(ActionEvent ae) {
				...
				if (newWidthVal <= 0) { // Resets textfield back to current value if user input is invalid
					...
					scaleCanvas((float) (stage.getWidth() - gui.toolBarWidth - gui.controlBarWidth),
							(float) (stage.getHeight() - 2 * gui.mb.getHeight()));
			...
			if (file != null) {
				...
				if (loadedImage.width > pad.getWidth() || loadedImage.height > pad.getHeight()) {
					...
					final Label text = new Label(
							"The reference image is larger than the canvas.\nHow would you like to resolve this?");
		...
		gui.colorPickerFill.setOnAction(new EventHandler() {
			public void handle(Event t) {
				...
				if (pad.isDrawMode()) {
					...
					tasks.push(new Change(Transformation.FIL, pad.listIndex,
							gui.shapes.get(pad.listIndex).getFillColor()));
		...
		gui.colorPickerStroke.setOnAction(new EventHandler() {
			public void handle(Event t) {
				...
				if (pad.isDrawMode()) {
					...
					tasks.push(new Change(Transformation.STF, pad.listIndex,
							gui.shapes.get(pad.listIndex).getStrokeColor()));
		...
		gui.weightSlider.valueProperty().addListener(new ChangeListener<Number>() {
			public void changed(ObservableValue<? extends Number> changed, Number oldVal, Number newVal) {
				...
				if (pad.isEditMode()) {
					tasks.push(
							new Change(Transformation.STW, pad.listIndex, gui.shapes.get(pad.listIndex).strokeWeight));
		...
		gui.btnReset.setOnAction(event -> {
			if (!gui.shapes.isEmpty()) {
				tasks.push(
						new Change(Transformation.RES, pad.listIndex, gui.shapes.get(pad.listIndex).getResetFloats()));
	...
	public void mousePressed() {
		activeButton = mouseButton; // stores a copy because mouseButton drops value before triggering
	...
	@Override
	// Check mouse LEFT vs RIGHT against "activeButton" because mouseButton variable
	// drops its value before these checks run
	...
	// Uses change operation and stored information to revert shape back to previous
	// state.
	void undo(Change task) {
		switch (task.getOperation()) {
		case ROT:
			gui.shapes.get(task.getIndex()).setRotation(task.changedValues[0]);
			dialog = "Reverted rotation.";
			break;
		case TRA:
			gui.shapes.get(task.getIndex()).manipulate(task.changedValues[0], task.changedValues[1]);
			dialog = "Reverted translation.";
			break;
		case SCA:
			gui.shapes.get(task.getIndex()).setHandles(task.changedValues);
			dialog = "Reverted scaling.";
			break;
		case DEL:
			gui.shapes.add(task.getClone());
			dialog = "Reverted deletion.";
			break;
		case ORD:
			swapElements((int) task.changedValues[0], (int) task.changedValues[1]);
			dialog = "Reverted reordering.";
			break;
		case ADD:
			deleteShape(task.getIndex());
			dialog = "Reverted addition of new shape.";
			break;
		case COP:
			deleteShape(task.getIndex());
			dialog = "Reverted copying of shape.";
			break;
		case RES:
			gui.shapes.get(task.getIndex()).setHandles(task.changedValues);
			...
			break;
		case FIL:
			gui.shapes.get(task.getIndex()).setFillColor((int) task.changedValues[0]);
			break;
		case STF:
			gui.shapes.get(task.getIndex()).setStrokeColor((int) task.changedValues[0]);
			break;
		case STW:
			gui.shapes.get(task.getIndex()).setStrokeWeight((int) task.changedValues[0]);
			break;
		}
	}
	...
	void swapElements(int currentIndex, int newIndex) {
		if (newIndex < 0 || newIndex == gui.shapes.size())
			return;
	...
	void deleteShape(int target) {
		if (pad.modifying)
			pad.completeShape(); // finishes polygon if interrupted while drawing
		if (!gui.shapes.isEmpty()) {
	...
	void drawMouse() {
		...
		case MOUSE:
			...
			break;
		case TOP:
			text("X: " + canvasX + ", Y: " + canvasY, width * horizontalPadding,
					height * verticalPadding - gui.spacing);
			break;
		}
	}
	...
	void generateCanvasArea() {
		...
		// Converts mouseX and mouseY to match canvas. (ie 0,0 mouse position is top
		// left of canvas)
	...
	// Scales canvas based on how much of the central screen remains after padding
	// is removed
	...
	void exportImageFile(String location) {
		...
		if (pad.getBackgroundColor() != gui.NONE)
			exportGraphic.background(pad.getBackgroundColor());
		for (int i = 0; i < gui.shapes.size(); i++) {
	...
	// Parsing should split first by ';' and the first element ("Bezier", "Circle",
	// "Rectangle", etc)
	...
	void saveDrawing(String location) {
		...
		try {
			...
			output.println(pad.getWidth() + "," + pad.getHeight() + "," + pad.getBackgroundColor() + ","
					+ pad.getGridDensity());
			for (int i = 0; i < gui.shapes.size(); i++) {
